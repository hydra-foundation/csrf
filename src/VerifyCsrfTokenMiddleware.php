 ...
 *
 * Safe methods (GET/HEAD/OPTIONS — the read-only ones) pass straight through:
 * they cause no side effects, and it is a GET rendering a form that mints the
 * token in the first place. An unsafe method (POST/PUT/PATCH/DELETE) must carry
 * a token matching the session's, or the request is rejected with a 403
...
{
    /** Methods that mutate state and therefore must carry a valid token. */
    ...
    {
        if (
            in_array(strtoupper($request->getMethod()), self::UNSAFE, true)
